...
test('Places word "fill" on grid', () => {
    ...
    expect(displayPlacement(placeWord("wow"))).toMatchInlineSnapshot(`
      ...
      "
    `);
    expect(displayPlacement(placeWord("wwww"))).toMatchInlineSnapshot('"Could not fit word into table"');
    expect(() => placeWord("12")).toThrowErrorMatchingInlineSnapshot('"1 is not defined in the letter map"');
    expect(displayPlacement(placeWord("wow!"))).toMatchInlineSnapshot(`
    "
    □                                 ■
    □ □                     □ □ □ □ □ ■
